...
| Navigate | Scroll to pan, Ctrl/⌘+scroll or pinch to zoom, Space+drag to pan, `+`/`-`, Shift+0 (100%), Shift+1 (fit), Shift+2 (fit selection). The view pill above the canvas shows the zoom and the drawing scale, snapped to a conventional value (≈ when it is only close); click the zoom for Zoom to fit, Zoom to selection and 25–400 % presets, or the scale to zoom to 1:20, 1:50, 1:100… exactly. |
