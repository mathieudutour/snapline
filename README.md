...
| Finishes | Select a room to set its floor (oak, walnut, tiles, carpet, concrete…) and wall paint or tiles; the Finishes tab of the left panel is the schedule: every room in reading order, plus the exterior wall and roof materials. Selecting a wall lets you override its finish. Textures are drawn procedurally and tile at real sizes; the 2D plan tints rooms by their floor. |
| Site and sun | Project menu → Settings… (or click the north arrow on the canvas) takes the building's latitude and longitude ("Use my location" fills them in), the compass bearing of the top of the plan and the slab thickness between floors; a north arrow then shows on the 2D canvas. In 3D and walkthrough, pick the season (June solstice, equinox, December solstice) and drag the time of day to see where the real sun stands and how the shadows move; night dims the scene. |
