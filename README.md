...
selection on the right. With nothing selected the inspector shows the floor's defaults, its underlay, and a
status list for the roof, the finishes and the site.
...
| Navigate | Scroll to pan, Ctrl/⌘+scroll or pinch to zoom, Space+drag to pan, `+`/`-`, Shift+0 (100%), Shift+1 (fit), Shift+2 (fit selection). The view pill above the canvas shows the zoom; click it for Zoom to fit, Zoom to selection and 25–400 % presets. The drawing scale sits in the bottom-right corner beside the north arrow, snapped to a conventional value (≈ when it is only close); click it to zoom to 1:20, 1:50, 1:100… exactly. |
| Measurements shown | Shift+D (or the pill item) cycles the density: **Overall** draws the building's outside size on each side plus room names; **Working** (the default) adds a chained string outside each side that reads wall, clear, wall, clear (ticks at the wall faces, so every number is one a tape can check), the locked and violated lengths, and free lengths on hover; **All** dimensions every wall face and opening. The selection is always dimensioned. At every level, two numbers never overlap: a label that would sit on another is dropped for a small dot that shows its value on hover, and a bay or room too small for its number gets a leader out to the margin. |
...
| Underlay (trace a plan) | With nothing selected, the floor section of the inspector has an "Import an underlay…" row that takes an image or a PDF (first page). The row then shows the image, its opacity and whether it is locked; open it for rotation, Scale (click two points a known distance apart and type the distance), Replace and Remove. Drag the image into place while unlocked, lock it and draw walls on top. The image is kept in the browser and copied to the account when signed in, so collaborators see it too. |
...
| Project objects | The Project group at the top of the left panel lists the three objects a project has beside its floors, each with its current value: **Building** (floors, exterior finish, the slab between floors, total height), **Roof** (type, pitch, ridge, overhang, finish; it sits on the top floor) and **Site** (location, north, sun). Select one and edit it in the inspector like a wall; Esc or a click on empty plan deselects. |
...
| Projects | Your plans live at `/projects` (the root sends you there when signed in) and each one is edited at `/project/<id>`, so a plan's URL can be bookmarked or shared with a collaborator. The floor you are on and what you have selected are part of the URL too (`/project/<id>/<floor>?s=wall:w1`), and opening such a link zooms to the selection. The project name at the top of the left panel opens a menu (rename, import, export, delete); the Projects rail tab goes back to the list. |
...
| Finishes | Select a room to set its floor (oak, walnut, tiles, carpet, concrete…) and wall paint or tiles; the Finishes tab of the left panel is the schedule: every room in reading order, its name selecting the room. The exterior finish is on the Building and the roof's on the Roof. Selecting a wall lets you override its finish. Textures are drawn procedurally and tile at real sizes; the 2D plan tints rooms by their floor. |
| Site and sun | Select Site (in the Project group, or by clicking the north arrow on the plan) and paste the building's coordinates from Google Maps as-is into Location, or use "Locate me". North is the compass bearing of the top of the plan: drag the compass needle in the inspector, or drag the north arrow on the plan while Site is selected. Once located, the Sun section picks the season (June solstice, equinox, December solstice) and the time of day; in 3D and walkthrough the real sun lights the scene and moves the shadows, and night dims it. |
